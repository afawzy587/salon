...
//	}
	...
	function getsalonsInformation()
	{
		if($GLOBALS['login']->doCheck() == true)
		{
			$query = $GLOBALS['db']->query("SELECT s.* , u.user_name FROM `".$this->tableName."` s INNER JOIN `users` u ON s.`owner_id` = u.`user_serial`  WHERE s.`salon_serial` = '1' LIMIT 1 ");
			$queryTotal = $GLOBALS['db']->resultcount();
			if($queryTotal > 0)
			{
